...
import { getSubjectDetail, aLevelSubjects, levelLabels } from "@/lib/subjects";
...
export default function ALevelSubjectDetailPage({
  ...
  const exists = aLevelSubjects.some((s) => s.slug === params.subject);
  if (!exists) notFound();

  const subject = getSubjectDetail("a-level", params.subject);
