...
### Signature Component: Asymmetric Hero with Crest Parallax
The hero (`the-infinite-grid.tsx`) is a two-column split (`md:grid-cols-[1.1fr_0.9fr]`), not a centered stack — this is a deliberate departure from the generic centered-eyebrow-headline-subtext-CTA template, per the brand register's anti-center-bias guidance. Left column: eyebrow, headline, subhead, CTA, all left-aligned. Right column (hidden below `md`, since mouse parallax has no meaning on touch and the column would otherwise just be empty weight): the brand crest (`public/crest.png`, cropped from the logo lockup, not a separately-designed asset) rendered at low opacity (0.14) as a midground layer, offset by a `useTransform`-driven parallax tied to normalized cursor position (±18px x, ±14px y), independent of viewport size. This is what turns the page's one interactive effect from generic "grid that reacts to your mouse" into something that's specifically this brand's mark, not borrowed dev-tool visual language. Disabled under `prefers-reduced-motion` (the crest still renders, just without the offset).

Behind both columns: a mouse-reactive SVG grid with a radial mask that brightens near the cursor, plus two static yellow glow orbs (yellow/35 and yellow-deep/25 — bumped up from the original dark-surface values, since light-on-light glows need more strength to read) for depth. Grid lines render in `ink` at low opacity rather than `mist`, so the texture is a soft graphite grid on white instead of disappearing. Grid scroll animation is disabled under `prefers-reduced-motion`.

### Named Rules
**The Asymmetry Rule.** The hero is a left-text/right-visual split, not a centered stack. If a future hero or section reverts to dead-center symmetry, that's a regression to the generic template shape this system explicitly broke away from — it needs a deliberate reason (e.g. a manifesto-style page where centered IS the right call), not a default reach.
